<script>
	import { db,auth,googleProvider } from './firebaseApp';
	...
	const loginWithGoogle = async () =>{
		try{
			...
			$user = res.user;
			$isLoggedIn = true;
			db.collection("user").doc(res.user.uid).collection("cart").add({
				timestamp: firebase.firestore.FieldValue.serverTimestamp(),
			});
			db.collection("user").doc(res.user.uid).collection("fav").add({
				timestamp: firebase.firestore.FieldValue.serverTimestamp(),
			});
